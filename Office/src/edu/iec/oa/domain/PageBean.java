...
public class PageBean {
	...
	public PageBean(int currentPage, int pageSize, int recordCount, List recordList) {
		...
		// 计算 pageCount:页数=（总记录数+每页大小-1）/每页大小    ；这样就是整数也了
		...
		}else{//总记录数大于10页，则显示当前页附近的10个页面
			//当前页附近共10个页码（前4+当前页+后5）
			beginPageIndex = currentPage - 4;
			endPageIndex = currentPage + 5;
			if(beginPageIndex < 1){//当前面的页面不足4个时：则显示前10个页码
				beginPageIndex = 1;
				endPageIndex = 10;
			}
			if(endPageIndex > pageCount){//当后面的页面不足5个时：则显示后10个页码
